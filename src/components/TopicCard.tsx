...
export function TopicCard({ topic, previewLines, onOpen }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="flex w-full min-w-0 flex-col items-start rounded-2xl border border-line bg-surface p-4 text-left shadow-[0_1px_2px_rgba(33,27,51,0.04)] transition-shadow hover:shadow-[0_6px_20px_rgba(107,78,255,0.10)]"
    >
      <div
        ref={titleRef}
      ...
      {preview.length > 0 && (
        <div className="mt-3 flex w-full flex-col gap-1.5 border-t border-line pt-3">
          {preview.map((t) => (
            <div key={t.id} className="flex min-w-0 items-center gap-2 text-xs text-muted">
              <ThoughtGlyph type={t.type} done={t.done} />
              <span className={`min-w-0 truncate ${t.type === 'check' && t.done ? 'text-faint line-through' : ''}`}>
