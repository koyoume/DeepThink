...
## 5. 열린 항목 / 추후 결정
- **하단 "새 생각 추가" 입력창 Enter 시 포커스 문제(확인 대기)**: Enter 치면 새 줄이 목록에 추가되는데 포커스가 그 줄로 튀어서, 입력창에 남아 연달아 입력하는 흐름이 끊기는 것으로 보임(§5.4 작업 중 사용자가 다른 요청으로 넘어가며 아직 확정 답변 대기 — "하단 입력창 포커스 유지" / "목록 안 Enter" / "둘 다" 중 미정). 확정되면 `submitDraft`/`addAtEnd`의 `setFocusRequest` 로직 수정 필요.
