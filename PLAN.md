...
- **롱프레스 드래그 학습**: 텍스트 있는 요소에 롱프레스 드래그를 붙일 땐 `touch-action:none`만으론 부족, `user-select:none` + `-webkit-touch-callout:none`도 항상 같이 걸어야 함(세션14에서 카테고리 칩에 이게 빠져 "누르면 글자 선택" 버그 발생). 직접 구현한 롱프레스+리플로우 드래그가 두 라운드째 실사용 버그를 냈으므로, 계속 재발하면 @dnd-kit 도입 고려(사용자에게 선택지로 제시됨, 3차 수정으로 원인 특정되어 보류 중).
