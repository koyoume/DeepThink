...
- **롱프레스 드래그 학습**: (1) 텍스트 있는 요소에 롱프레스 드래그를 붙일 땐 `touch-action:none`만으론 부족, `user-select:none` + `-webkit-touch-callout:none`도 항상 같이 걸어야 함(세션14). (2) 롱프레스 대기 타이머는 `pointerup`에서 **리스너 정리뿐 아니라 `clearTimeout`도 반드시 호출**해야 함 — 안 그러면 짧은 탭 후에도 타이머가 그대로 실행돼 드래그가 뒤늦게 발동함(세션14 4차 수정, 카드 겹침 재발의 실제 원인이었을 가능성 큼). 코드 리뷰 시 "대기 타이머를 쓰는 모든 pointerup 핸들러에 clearTimeout이 있는지" 체크리스트화.
