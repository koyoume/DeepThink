...
## 5.8 접속 시 선택된 카테고리 칩 자동 센터링 (2026-07-19, 16차 수정)
카테고리 칩 행은 가로 스크롤(`overflow-x-auto`)이라 칩이 많으면 선택된 칩이 화면 밖(주로 오른쪽)에 있을 수 있다. 선택 하이라이트는 켜져 있어도, 접속 직후엔 그 칩이 보이지 않아 칩 행을 직접 밀어봐야 어디가 선택됐는지 알 수 있던 문제.
