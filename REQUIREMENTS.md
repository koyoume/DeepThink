...
## 5.4 왼쪽 글리프 클릭으로 타입·완료 상태 순환 — 타입 전환 메뉴 폐기 (2026-07-05, 11차 수정)
줄 왼쪽 글리프(체크박스/코멘트 아이콘)를 클릭하면 **코멘트 → 체크(미완료) → 체크(완료) → 코멘트** 순으로 한 번씩 클릭할 때마다 순환한다. 기존엔 "완료/미완료"만 왼쪽 클릭으로 되고, "체크↔코멘트" 타입 전환은 오른쪽 ⋯ 메뉴에 따로 있었는데, 이 메뉴 항목을 없애고 왼쪽 글리프 클릭 하나로 통합.

- **`useTopicDetailState.ts`**: `toggleDone`/`setType`/`toggleNewType`/`toggleType`/`newType` 상태를 전부 제거하고, 3단계 순환을 처리하는 `cycleType(id)` 하나로 통합.
- **`ThoughtGlyph.tsx`**: prop `onToggle` → `onCycle`로 변경, 코멘트 글리프도 이제 클릭 가능한 버튼(이전엔 체크 타입만 클릭 가능했음). `aria-label`도 "클릭하면 무엇으로 바뀌는지" 안내하도록 갱신.
- **`ThoughtRow.tsx`**: ⋯ 메뉴에서 "코멘트로 전환"/"체크로 전환" 항목 제거(들여쓰기·내어쓰기·삭제는 유지).
- **하단 "새 생각 추가" 입력바**: 타입을 미리 고르는 "체크/코멘트" 토글 버튼 제거 — 이제 아무 타입이나 추가한 뒤 글리프 클릭으로 바꾸면 되므로 사전 선택이 불필요해짐. `addAtEnd`는 항상 체크(미완료)로 시작(가장 흔한 사용 패턴).
- **영향 없음**: `TopicCard.tsx`의 미리보기 글리프는 원래도 클릭 불가(읽기 전용) 상태라 변경 없음.

## 5.5 하단 "새 생각 추가" 입력바 폐기 — 일반 에디터처럼 동작 (2026-07-05, 12차 수정)
"일반 편집기엔 하단 입력바가 따로 있은 적이 있냐"는 지적으로 재검토 — 맞는 말이라 별도 입력바 자체를 없앴다. 마지막 줄에서 Enter를 치면 그 아래 새 줄이 생기는 기존 `addAfter` 동작만으로 충분하고, 이게 일반 텍스트 에디터의 자연스러운 흐름과 같다.

- **삭제**: 화면 하단에 고정돼 있던 "새 생각 추가" `<input>`과 `submitDraft`/`draft` 상태 전부 제거.
- **보강**: 목록이 완전히 비면(신규 주제를 처음 열었을 때, 또는 마지막 줄까지 다 지웠을 때) 커서를 둘 빈 줄 하나를 자동으로 만든다(`useEffect`가 `thoughts.length === 0`을 감지해 `addAtEnd()` 호출) — 빈 문서를 열어도 커서가 있는 것과 같은 느낌. 이 자동 생성 줄도 그냥 보통 줄이라 타입 순환(§5.4)·들여쓰기·삭제 다 똑같이 된다.
- **하단 여백 축소**: 고정 푸터가 없어졌으니 목록 컨테이너의 `pb-24` → `pb-8`로 축소.
- **정리**: 더 이상 쓰지 않는 `EmptyState`(빈 목록 안내 문구, 하단 입력창을 가리키던 문구라 의미도 없어짐) import 제거.
- **배포 실패 후속 수정**: 12차 수정 커밋이 실제로는 **E2E 게이트(Playwright)에서 막혀 Cloudflare Pages에 배포되지 않았음** — `e2e/dashboard-flow.spec.ts`가 옛 UI(하단 입력바, 옛 `pb-24` 클래스 셀렉터) 기준으로 작성돼 있었는데 갱신을 안 해서 실패. `ThoughtRow.tsx`의 뷰 버튼·입력창에 `data-testid="thought-view"`/`"thought-input"`을 추가해 Tailwind 클래스 변경에 흔들리지 않는 안정적인 셀렉터로 바꾸고, 테스트 시나리오도 "마지막 줄에서 Enter로 새 줄 추가"로 다시 작성. 이 환경엔 브라우저가 없어 Playwright를 직접 돌려볼 수 없으므로, GitHub Actions CI 결과로 최종 확인 필요.
